...
describe("Category Integration Tests", () => {
    describe("Create method", () => {
        ...
        it("Should throw error with a invalid category is_active when create", () => {
            expect(
                () =>
                    new Category({ is_active: 5 as any, name: "name valido" }),
            ).containsErrorMessages({
    ...
    describe("Update method", () => {
        it("Should throw error with a invalid category name when create", () => {
            ...
            expect(
                () => new Category({ name: null, description: null }),
            ).containsErrorMessages({
                name: [
                    "name should not be empty",
                    "name must be a string",
            ...
            expect(
                () => new Category({ name: "", description: null }),
            ).containsErrorMessages({
                name: ["name should not be empty"],
            });

            expect(
                () => new Category({ name: 5 as any, description: null }),
            ).containsErrorMessages({
                name: [
                    "name must be a string",
                    "name must be shorter than or equal to 255 characters",
            ...
            expect(
                () =>
                    new Category({ name: "t".repeat(256), description: null }),
            ).containsErrorMessages({
                name: ["name must be shorter than or equal to 255 characters"],
            });
        });
        ...
        it("Should throw error with a invalid category name when create", () => {
            ...
            expect(() =>
                category.update("Valid", 5 as any),
            ).containsErrorMessages({
                description: ["description must be a string"],
            });
        });
